...
global using WH.WisdomVisionPro.NodeGroup.Groups.Others;
global using WH.WisdomVisionPro.ResultPresenter;
global using WH.WisdomVisionPro.ResultPresenter.ResultPresenters;
...
public class Yolov3 : OpenCVNodeDataBase, IOtherGroupableNodeData
{
    public override void LoadDefault()
    {
        ...
        this.WeightFilePath = @"Yolov\\yolov3-tiny.weights".ToAssetsPath();
    }

    private string _cfgFilePath;
    [Required]
    [Display(Name = "Cfg路径", GroupName = VisionPropertyGroupNames.RunParameters)]
    ...
    [Required]
    [Display(Name = "Weight路径", GroupName = VisionPropertyGroupNames.RunParameters)]
    [PropertyItem(typeof(OpenFileDialogPropertyItem))]
    ...
    protected override async Task<IFlowableResult> BeforeInvokeAsync(IFlowableLinkData previors, IFlowableDiagramData diagram)
    {
        if (!File.Exists(this.WeightFilePath) || !File.Exists(this.CfgFilePath) || !File.Exists(this.NameFilePath))
        {
            ...
            {
                return await IocMessage.Form?.ShowEdit(this, x => x.Title = $"{this.Name}:模型文件不能为空，请先选择一个模型文件", null, x =>
                 {
    ...
    protected override FlowableResult<Mat> Invoke(ISrcVisionNodeData<Mat> srcImageNodeData, IVisionNodeData<Mat> from, IFlowableDiagramData diagram)
    {
        ...
        List<Tuple<string, double, System.Windows.Rect>> rects = new List<Tuple<string, double, System.Windows.Rect>>();
        foreach (int i in indices)
        {
            ...
            //画方框
            org.Rectangle(new Point(x1, box.Y - box.Height / 2), new Point(box.X + box.Width / 2, box.Y + box.Height / 2), VisionSettings.Instance.OutputColor.ToScalar(), org.ToThickness());

            //标签字符大小
            Size textSize = Cv2.GetTextSize(label, HersheyFonts.HersheyTriplex, 0.5, 1, out int baseline);
            //画标签背景框
            Rect rect = new Rect(new Point(x1, box.Y - box.Height / 2 - textSize.Height - baseline),
                new Size(textSize.Width, textSize.Height + baseline));
            Cv2.Rectangle(org, rect, VisionSettings.Instance.OutputColor.ToScalar(), Cv2.FILLED);
            Cv2.PutText(org, label, new Point(x1, box.Y - box.Height / 2 - baseline), HersheyFonts.HersheyTriplex, 0.5, VisionSettings.Instance.OutputLabelColor.ToScalar());

            rects.Add(Tuple.Create(lables[classIds[i]], Math.Round(probabilities[i], 2), new System.Windows.Rect(x1, box.Y - box.Height / 2, box.Width, box.Height)));
        }
        this.MatchingCountResult = boxes.Count;
        H.Controls.Diagram.Presenter.NodeDatas.Base.IResultPresenter resultPresenter = rects.ToResultPresenter();
        return this.OK(org, resultPresenter, this.MatchingCountResult.ToDetectSuccessMessage());
    }
}
